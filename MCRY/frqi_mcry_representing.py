...
class FRQI_MCRY_Simulator:
    ...
    def mcry_gate(self, qc, theta, phi=0, lam=0, gamma=0, control_qubits=None, target_qubit=None):
        """
        Implementa la descomposición mostrada en el diagrama,
        usando una lista de qubits de control.
    
        Args:
            qc (QuantumCircuit): El circuito cuántico.
            theta (float): El ángulo de rotación para las compuertas U.
            control_qubits (list): Una lista de los qubits de control.
            target_qubit (int): El índice del qubit objetivo.
        """

        #Primera Compuerta controlada CU
        qc.cu(theta/2, phi, lam, gamma ,control_qubits[0], target_qubit)

        #Aplicando el primer CNOT
        qc.cx(control_qubits[0], control_qubits[1])


        #Segunda Compuerta controlada CU
        qc.cu(-theta/2, phi, lam, gamma, control_qubits[1], target_qubit)

        #Aplicando el segundo CNOT
        qc.cx(control_qubits[0], control_qubits[1])

        #Tercera compuperta controlada CU
        qc.cu(theta/2, phi, lam, gamma, control_qubits[1], target_qubit)
